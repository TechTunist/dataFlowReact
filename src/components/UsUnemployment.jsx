...
const UsUnemploymentChart = ({ isDashboard = false }) => {
    ...
    const { unemploymentData, fetchUnemploymentData } = useContext(DataContext);
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (unemploymentData.length > 0) return;
            setIsLoading(true);
            setError(null);
            try {
                await fetchUnemploymentData();
            } catch (err) {
                setError('Failed to fetch unemployment data. Please try again later.');
        ...
        fetchData();
    }, [fetchUnemploymentData, unemploymentData.length]);
    ...
    useEffect(() => {
        ...
        areaSeries.setData(unemploymentData);
    ...
    }, [unemploymentData, colors, scaleMode, theme.palette.mode]);
    ...
    return (
        <div style={{ height: '100%' }}>
            ...
            <div className='under-chart'>
                {!isDashboard && unemploymentData.length > 0 && (
                    <div style={{ marginTop: '10px' }}>
                        <span style={{ color: colors.greenAccent[500] }}>Last Updated: {unemploymentData[unemploymentData.length - 1].time}</span>
                    </div>
                )}
            ...
            {!isDashboard && tooltipData && (
                <div className="tooltip" style={{
                    ...
                    <div style={{ fontSize: '15px' }}>Unemployment</div>
            ...
            {!isDashboard && (
                <p className='chart-info'>
                    This chart shows the historical unemployment rate of the United States.
                </p>
            )}
        </div>
    );
...
export default UsUnemploymentChart;
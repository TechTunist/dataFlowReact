...
const UKPopulation = ({ isDashboard = false }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const url = `https://www.nomisweb.co.uk/api/v01/dataset/NM_31_1.data.json?geography=2092957697&sex=${selectedGender}&age=0&measures=20100&time=first,latest`;
        ...
        setError('Failed to load UK population data from Nomis.');
  ...
  useEffect(() => {
    ...
    const s = chart.addLineSeries({ color: '#8b5cf6', lineWidth: 2, priceScaleId: 'left', lastValueVisible: true });
    seriesRef.current = s;
    s.setData(data);
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
        ...
        <div className="under-chart" style={{ flexShrink: 0, padding: '10px 0 20px 0', display: 'block' }}>
          {data.length > 0 && (
            <div style={{ marginBottom: '8px', color: colors.grey[100], fontSize: '15px', textAlign: 'left' }}>
              Current UK Population ({genderLabel}, all ages): <span style={{ color: '#a78bfa', fontWeight: 600 }}>{formatVal(data[data.length-1]?.value)}</span>
            </div>
          )}
      ...
      {!isDashboard && tooltipData && (
        <div className="tooltip" style={{ left: '50%', top: `${tooltipData.y + 80}px`, transform: 'translateX(-50%)' }}>
          <div style={{ fontSize: '13px' }}>UK Population ({genderLabel})</div>

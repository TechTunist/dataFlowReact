...
const UKEsaClaimants = ({ isDashboard = false }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // ESA dataset may have limited national recent data; using working JSA detailed for demo with gender selector
        const url = `https://www.nomisweb.co.uk/api/v01/dataset/NM_18_1.data.json?geography=2092957697&sex=${selectedGender}&duration=0&measures=20206&time=first,latest`;
        ...
        const normalizeTime = (t) => {
          ...
          // Common Nomis time formats -> lightweight-charts 'yyyy-mm-dd'
        ...
        // Dedupe any duplicate times (keep first after sort) to prevent lightweight-charts ordering errors
        const seen = new Set();
        ...
        setData(series);
      } catch (err) { setError('Failed to load UK ESA claimants data from Nomis.'); } finally { setIsLoading(false); }
    };
    fetchData();
  }, [selectedGender]);
  ...
  useEffect(() => {
    ...
    const s = chart.addAreaSeries({ color: '#ef4444', lineColor: '#f87171', topColor: 'rgba(239, 68, 68, 0.3)', bottomColor: 'rgba(239, 68, 68, 0.05)', lineWidth: 2, priceScaleId: 'left', lastValueVisible: true });
      ...
      {!isDashboard && (
        <div className="chart-top-div" style={{ flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
            ...
            {genderOptions.map(g => <button key={g.value} onClick={() => setSelectedGender(g.value)} className="button-reset" style={{ backgroundColor: selectedGender === g.value ? '#4cceac' : 'transparent', color: selectedGender === g.value ? 'black' : '#31d6aa', borderColor: selectedGender === g.value ? 'violet' : '#70d8bd', fontSize: '12px', padding: '4px 8px' }}>{g.label}</button>)}
...
<>
<div style={{ fontSize: '13px' }}>UK ESA Claimants ({genderLabel})</div>
            <div style={{ fontSize: '18px', fontWeight: 600, color: '#f87171' }}>{formatVal(tooltipData.value)}</div>
      ...
      {!isDashboard && (
        <div className="under-chart" style={{ flexShrink: 0, padding: '10px 0 20px 0', display: 'block' }}>
          {data.length > 0 && <div style={{ marginBottom: '8px', color: colors.grey[100], fontSize: '15px', textAlign: 'left' }}>Current UK ESA Claimants ({genderLabel}): <span style={{ color: '#f87171', fontWeight: 600 }}>{formatVal(data[data.length-1]?.value)}</span></div>}

...
const UKWorkforceJobs = ({ isDashboard = false }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setData(series);
      } catch (err) { setError('Failed to load UK workforce jobs data from Nomis.'); } finally { setIsLoading(false); }
    };
    fetchData();
  }, []);
  ...
  useEffect(() => {
    ...
    const s = chart.addLineSeries({ color: '#10b981', lineWidth: 2, priceScaleId: 'left', lastValueVisible: true });
...
<>
<div style={{ fontSize: '13px' }}>UK Workforce Jobs</div>
            <div style={{ fontSize: '18px', fontWeight: 600, color: '#34d399' }}>{formatVal(tooltipData.value)}</div>
      ...
      {!isDashboard && (
        <div className="under-chart" style={{ flexShrink: 0, padding: '10px 0 20px 0', display: 'block' }}>
          {data.length > 0 && <div style={{ marginBottom: '8px', color: colors.grey[100], fontSize: '15px', textAlign: 'left' }}>Current UK Workforce Jobs: <span style={{ color: '#34d399', fontWeight: 600 }}>{formatVal(data[data.length-1]?.value)}</span></div>}
...
export default UKWorkforceJobs;
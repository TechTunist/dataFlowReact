...
const UKEarnings = ({ isDashboard = false }) => {
  ...
  // Gender options for division (SEX dimension)
  const genderOptions = [
    { value: '7', label: 'Total' },
    { value: '5', label: 'Male' },
    { value: '6', label: 'Female' },
  ];
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        // NM_30_1 ASHE, item=2 median, measures for pay value, gender
        const url = `https://www.nomisweb.co.uk/api/v01/dataset/NM_30_1.data.json?geography=2092957697&sex=${selectedGender}&item=2&pay=1&measures=20100&time=first,latest`;
        const res = await fetch(url);
        if (!res.ok) throw new Error('Nomis error');
        ...
        setError('Failed to load UK earnings data from Nomis.');
        console.error(err);
      } finally {
        setIsLoading(false);
      }
  ...
  useEffect(() => {
    ...
    const chart = createChart(chartContainerRef.current, {
      ...
      leftPriceScale: { visible: true, borderVisible: false, mode: 0, scaleMargins: { top: 0.1, bottom: 0.1 } },
      crosshair: { mode: 1 },
    });
    ...
    chart.subscribeCrosshairMove((param) => {
      if (!param.point || !param.time) { setTooltipData(null); return; }
      const d = param.seriesData.get(s);
      setTooltipData({ date: param.time, value: d?.value, x: param.point.x, y: param.point.y });
    });
  ...
  const formatVal = (v) => v != null ? '£' + v.toFixed(0) : 'N/A';
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      {!isDashboard && (
        <div className="chart-top-div" style={{ flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
            <span style={{ color: colors.grey[100], fontSize: '12px' }}>Gender:</span>
            {genderOptions.map(g => (
              <button key={g.value} onClick={() => setSelectedGender(g.value)} className="button-reset" style={{ backgroundColor: selectedGender === g.value ? '#4cceac' : 'transparent', color: selectedGender === g.value ? 'black' : '#31d6aa', borderColor: selectedGender === g.value ? 'violet' : '#70d8bd', fontSize: '12px', padding: '4px 8px' }}>
                {g.label}
              </button>
            ))}
            {['all', '10y', '5y', '2y'].map(r => (
              <button key={r} onClick={() => applyTimeRange(r)} className="button-reset" style={{ backgroundColor: timeRange === r ? '#4cceac' : 'transparent', color: timeRange === r ? 'black' : '#31d6aa', borderColor: timeRange === r ? 'violet' : '#70d8bd', fontSize: '12px', padding: '4px 8px' }}>
        ...
        <div className="under-chart" style={{ flexShrink: 0, padding: '10px 0 20px 0', display: 'block' }}>
          {data.length > 0 && (
            <div style={{ marginBottom: '8px', color: colors.grey[100], fontSize: '15px', textAlign: 'left' }}>
              Current UK Median Earnings ({genderLabel}): <span style={{ color: '#60a5fa', fontWeight: 600 }}>{formatVal(data[data.length-1]?.value)}</span>
            </div>
          )}
      ...
      {!isDashboard && tooltipData && (
        <div className="tooltip" style={{ left: '50%', top: `${tooltipData.y + 80}px`, transform: 'translateX(-50%)' }}>
          <div style={{ fontSize: '13px' }}>UK Earnings ({genderLabel})</div>
          <div style={{ fontSize: '18px', fontWeight: 600, color: '#60a5fa' }}>{formatVal(tooltipData.value)}</div>

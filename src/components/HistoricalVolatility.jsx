...
const BitcoinHistoricalVolatility = ({ isDashboard = false }) => {
  ...
  // Memoize raw data selection for stable references (prevents chart effects from
  // re-running on every DataContext churn that produces new array identities)
  ...
  // **Memoize volatility data and max volatility** (now depends on stable btcData ref)
  ...
  // Memoize derived price series data for stable deps in effects (avoids re-setData
  // and chart churn when context provides fresh array ref for unrelated reasons)
  const priceData = useMemo(() => btcData.map(data => ({ time: data.time, value: data.value })), [btcData]);
  ...
  // **Update price series data** (depends on stable memoized priceData)
  useEffect(() => {
  ...
  // **Update current price and volatility** (use stable priceData for latest price)
  useEffect(() => {

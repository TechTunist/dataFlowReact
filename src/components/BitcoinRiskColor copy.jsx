...
const FearAndGreedChart = ({ isDashboard = false }) => {
    ...
    const colors = tokens(theme.palette.mode);
    const [layout, setLayout] = useState({
        title: 'Bitcoin Price vs. Fear and Greed Index',
        autosize: true,
        margin: { l: 50, r: 50, b: 30, t: 30, pad: 4 },
        plot_bgcolor: colors.primary[700],
        paper_bgcolor: colors.primary[700],
    ...
    useEffect(() => {
        const fetchBitcoinData = async () => {
            ...
            const btcFormattedData = btcData.map(item => ({
                time: new Date(item.date * 1000), // Convert Unix timestamp to JavaScript Date object
                value: parseFloat(item.close)
            }));
            ...
            const fearGreedFormattedData = fearGreedData.map(item => ({
                time: new Date(item.timestamp * 1000), // Convert Unix timestamp to JavaScript Date object
                value: item.value,
            ...
            setDatasets([
                ...
                ...fearGreedFormattedData.map(d => ({
                    x: [d.time], // Convert single point into an array for plotting
                    y: [btcFormattedData.find(btc => btc.time.toISOString().slice(0, 10) === d.time.toISOString().slice(0, 10))?.value || null],
                    type: 'scatter',
                    mode: 'markers',
                    marker: {
                        size: 12,
                        color: getColor(d.value), // Function to determine color based on the Fear and Greed value
                    },
                    name: `Fear/Greed: ${d.classification}`
                }))
            ]);
        };
    ...
    const getColor = (value) => {
        // Return color based on the Fear and Greed index value
        if (value < 25) return '#FF4500'; // Red
    ...
    return (
        <div style={{ height: '100%' }}>
            <div className='chart-top-div'>
                {!isDashboard && (
                    <button onClick={() => setLayout({ ...layout, xaxis: { ...layout.xaxis, autorange: true }, yaxis: { ...layout.yaxis, autorange: true } })} className="button-reset">
                        Reset Chart
                    </button>
                )}
            </div>

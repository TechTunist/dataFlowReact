...
const UKJSAByEthnicity = ({ isDashboard = false }) => {
  ...
  const [selectedGender, setSelectedGender] = useState('7');
  const [selectedEthnicity, setSelectedEthnicity] = useState('0');

  const genderOptions = [{ value: '7', label: 'Total' }, { value: '5', label: 'Male' }, { value: '6', label: 'Female' }];
  // Ethnicity options from Nomis NM_118_1 (hardcoded common for functionality)
  const ethnicityOptions = [
    { value: '0', label: 'Total' },
    { value: '1', label: 'White - British' },
    { value: '8', label: 'Asian - Indian' },
    { value: '9', label: 'Asian - Pakistani' },
    { value: '12', label: 'Black - Caribbean' },
    { value: '13', label: 'Black - African' },
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Using England geo for ethnicity data availability (NM_118_1); label as detailed UK/England level
        const url = `https://www.nomisweb.co.uk/api/v01/dataset/NM_118_1.data.json?geography=2092957699&sex=${selectedGender}&ethnicity=${selectedEthnicity}&age=0&duration=0&measures=20100&item=1&time=first,latest`;
        ...
        setData(series);
      } catch (err) { setError('Failed to load UK JSA ethnicity data from Nomis (data available at England level for ethnicity).'); } finally { setIsLoading(false); }
    };
    fetchData();
  ...
  useEffect(() => {
    ...
    const s = chart.addAreaSeries({ color: '#f43f5e', lineColor: '#fb7185', topColor: 'rgba(244, 63, 94, 0.3)', bottomColor: 'rgba(244, 63, 94, 0.05)', lineWidth: 2, priceScaleId: 'left', lastValueVisible: true });
  ...
  const ethLabel = ethnicityOptions.find(e => e.value === selectedEthnicity)?.label || 'Total';
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      {!isDashboard && (
        <div className="chart-top-div" style={{ flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
            ...
            <span style={{ color: colors.grey[100], fontSize: '11px', marginLeft: '8px' }}>Ethnicity:</span>
            <select value={selectedEthnicity} onChange={e => setSelectedEthnicity(e.target.value)} style={{ fontSize: '11px', padding: '3px' }}>
              {ethnicityOptions.map(e => <option key={e.value} value={e.value}>{e.label}</option>)}
            </select>
      ...
      {!isDashboard && (
        <div className="under-chart" style={{ flexShrink: 0, padding: '10px 0 20px 0', display: 'block' }}>
          {data.length > 0 && <div style={{ marginBottom: '8px', color: colors.grey[100], fontSize: '15px', textAlign: 'left' }}>Current UK JSA Claimants ({genderLabel}, {ethLabel}): <span style={{ color: '#fb7185', fontWeight: 600 }}>{formatVal(data[data.length-1]?.value)}</span></div>}
      ...
      {!isDashboard && tooltipData && (
        <div className="tooltip" style={{ left: '50%', top: `${tooltipData.y + 80}px`, transform: 'translateX(-50%)' }}>
          <div style={{ fontSize: '13px' }}>UK JSA ({genderLabel}, {ethLabel})</div>
          <div style={{ fontSize: '18px', fontWeight: 600, color: '#fb7185' }}>{formatVal(tooltipData.value)}</div>
...
export default UKJSAByEthnicity;
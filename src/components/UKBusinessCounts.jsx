...
const UKBusinessCounts = ({ isDashboard = false }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Note: business counts dataset does not support SEX or ITEM; using totals for industry/size/legal
        const url = `https://www.nomisweb.co.uk/api/v01/dataset/NM_142_1.data.json?geography=2092957697&industry=37748736&employment_sizeband=0&legal_status=0&measures=20100&time=first,latest`;
        ...
        setData(series);
      } catch (err) { setError('Failed to load UK business counts data from Nomis.'); } finally { setIsLoading(false); }
    };
    fetchData();
  }, []);
  ...
  useEffect(() => {
    ...
    const s = chart.addLineSeries({ color: '#f59e0b', lineWidth: 2, priceScaleId: 'left', lastValueVisible: true });
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      {!isDashboard && (
        <div className="under-chart" style={{ flexShrink: 0, padding: '10px 0 20px 0', display: 'block' }}>
          {data.length > 0 && <div style={{ marginBottom: '8px', color: colors.grey[100], fontSize: '15px', textAlign: 'left' }}>Current UK Business Counts: <span style={{ color: '#f59e0b', fontWeight: 600 }}>{formatVal(data[data.length-1]?.value)}</span></div>}
      ...
      {!isDashboard && tooltipData && (
        <div className="tooltip" style={{ left: '50%', top: `${tooltipData.y + 80}px`, transform: 'translateX(-50%)' }}>
          <div style={{ fontSize: '13px' }}>UK Business Counts</div>
          <div style={{ fontSize: '18px', fontWeight: 600, color: '#f59e0b' }}>{formatVal(tooltipData.value)}</div>
...
export default UKBusinessCounts;
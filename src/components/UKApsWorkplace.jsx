...
const UKApsWorkplace = ({ isDashboard = false }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const url = `https://www.nomisweb.co.uk/api/v01/dataset/NM_100_1.data.json?geography=2092957697&measures=20100&time=first,latest`;
        ...
        setData(series);
      } catch (err) { setError('Failed to load UK APS workplace data from Nomis.'); } finally { setIsLoading(false); }
    };
    fetchData();
  }, []);
  ...
  useEffect(() => {
    ...
    const s = chart.addLineSeries({ color: '#ec4899', lineWidth: 2, priceScaleId: 'left', lastValueVisible: true });
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      {!isDashboard && tooltipData && (
        <div className="tooltip" style={{ left: '50%', top: `${tooltipData.y + 80}px`, transform: 'translateX(-50%)' }}>
          <div style={{ fontSize: '13px' }}>UK APS Workplace</div>
          <div style={{ fontSize: '18px', fontWeight: 600, color: '#ec4899' }}>{formatVal(tooltipData.value)}</div>

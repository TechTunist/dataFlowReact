...
const Total2Chart = ({ isDashboard = false }) => {
  ...
  const fetchTotal2Data = useCallback(async () => {
    try {
      setIsLoading(true);
      setError(null);
      const response = await fetch('https://vercel-dataflow.vercel.app/api/total2/');
      if (!response.ok) {
        throw new Error('Failed to fetch total2 data');
      }
      ...
      setTotal2Data(filteredData);
      setTotal2LastUpdated(new Date());
      
      return filteredData;
    } catch (err) {
      setError('Failed to fetch total2 data. Please try again later.');
      console.error('Error fetching total2 data:', err);
      return [];
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div style={{ height: '100%' }}>
      ...
      <div className="under-chart">
        {!isDashboard && total2Data.length > 0 && (
          <div style={{}}>
            {!isDashboard && <LastUpdated customDate={total2LastUpdated} />}
            {valuationDifference && (
              <span
                style={{
                  paddingTop: '20px',
              ...
              >
                {valuationDifference.label}: {valuationDifference.percentage} {valuationDifference.currentTotal2}
              </span>
            )}
          </div>
        )}
      ...
      {!isDashboard && (
        <div className="chart-info">
          Total2 metric displayed over time (from 2013-11-27 onwards). The regression bands have been fitted to the absolute lows, 

  ...
  useEffect(() => {
    ...
    const updateDatasets = (data) => {
      ...
      let newDatasets = riskBands.map((upperLimit, index) => {
        ...
        return {
          data: filteredData,
          visible: datasets[index]?.visible ?? true, // Preserve visibility
          type: 'scatter',
          mode: 'markers',
          marker: {
      ...
      newDatasets.push({
        ...
        visible: datasets[10]?.visible ?? true, // Preserve visibility
      });
      setDatasets(newDatasets);
    };
    updateDatasets(chartData);
  }, [chartData, theme, selectedAssetLabel, isNarrowScreen]);
  ...
  useEffect(() => {
    setLayout(prevLayout => ({
      ...
      legend: {
        ...prevLayout.legend,
        title: { text: isNarrowScreen ? '' : 'Select Risk Bands' },
        tracegroupgap: isNarrowScreen ? 1 : 10,
        itemsizing: isNarrowScreen ? 'constant' : undefined,
        font: { size: isNarrowScreen ? 10 : 12 },
      },
    }));
  }, [colors, selectedAssetLabel, isDashboard, isNarrowScreen]);
        ...
        >
          <Box
            sx={{
              ...
              width: '100%',
              padding: '0 0px',
              ...(isNarrowScreen && { flexWrap: 'nowrap' }),
            }}
          >
            <FormControl sx={{
              ...(isNarrowScreen ? { flex: 2, minWidth: '100px' } : { minWidth: '100px', width: { xs: '100%', sm: '200px' } }),
            }}>
              <InputLabel
                id="asset-label"
                shrink

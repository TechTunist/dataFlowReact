...
const TotalMarketCap = ({ isDashboard = false }) => {
    ...
    const { marketCapData, fetchMarketCapData } = useContext(DataContext);
    ...
    // Removed scaleMode state since it's now fixed to logarithmic (1)
    ...
    // Memoized regression lines calculation
    ...
    useEffect(() => {
        ...
        const logRegression2TopSeries = chart.addLineSeries({ color: 'lime', lineWidth: 2, lineStyle: 1, lastValueVisible: false, priceLineVisible: false });
        const logRegressionTopSeries = chart.addLineSeries({ color: 'green', lineWidth: 2, lineStyle: 1, lastValueVisible: false, priceLineVisible: false });
        const logRegressionMidSeries = chart.addLineSeries({ color: 'violet', lineWidth: 2, lineStyle: 1, lastValueVisible: false, priceLineVisible: false });
        const logRegressionBaseSeries = chart.addLineSeries({ color: 'red', lineWidth: 2, lineStyle: 1, lastValueVisible: false, priceLineVisible: false });
        const logRegressionBase2Series = chart.addLineSeries({ color: 'maroon', lineWidth: 2, lineStyle: 1, lastValueVisible: false, priceLineVisible: false });
    ...
    }, [marketCapData, colors, regressionLines]);

    // Update interactivity (removed scaleMode update since it's fixed)
    useEffect(() => {
        if (chartRef.current) {
    ...
    return (
        <div style={{ height: '100%' }}>
            {!isDashboard && (
                <div className='chart-top-div'>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                        {/* Static label instead of toggle switch */}
            ...
            <div className="chart-container" style={{ position: 'relative', height: isDashboard ? '100%' : 'calc(100% - 40px)', width: '100%', border: '2px solid #a9a9a9' }}>
                <div ref={chartContainerRef} style={{ height: '100%', width: '100%', zIndex: 1 }} onDoubleClick={setInteractivity} />
            </div>
            <div className='under-chart'>
                {!isDashboard && marketCapData.length > 0 && (
                    <div style={{ marginTop: '10px' }}>
                        <span style={{ color: colors.greenAccent[500] }}>Last Updated: {marketCapData[marketCapData.length - 1].time}</span>
                    </div>
                )}

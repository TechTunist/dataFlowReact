...
const UKJSAByAge = ({ isDashboard = false }) => {
  ...
  const ageOptions = [{ value: '0', label: 'Aged 16-64' }, { value: '100', label: 'Aged 16-24' }, { value: '200', label: 'Aged 25-49' }, { value: '300', label: 'Aged 50-64' }];

  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true); setError(null);
      try {
        const url = `https://www.nomisweb.co.uk/api/v01/dataset/NM_18_1.data.json?geography=2092957697&sex=${selectedGender}&age=${selectedAge}&duration=0&measures=20206&time=first,latest`;
        ...
        setData(series);
      } catch (err) { setError('Failed to load UK JSA by age data from Nomis.'); } finally { setIsLoading(false); }
    };
    fetchData();
  ...
  useEffect(() => {
    ...
    const s = chart.addAreaSeries({ color: '#eab308', lineColor: '#facc15', topColor: 'rgba(234, 179, 8, 0.3)', bottomColor: 'rgba(234, 179, 8, 0.05)', lineWidth: 2, priceScaleId: 'left', lastValueVisible: true });
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      {!isDashboard && (
        <div className="under-chart" style={{ flexShrink: 0, padding: '10px 0 20px 0', display: 'block' }}>
          {data.length > 0 && <div style={{ marginBottom: '8px', color: colors.grey[100], fontSize: '15px', textAlign: 'left' }}>Current UK JSA Rate ({genderLabel}, {ageLabel}): <span style={{ color: '#eab308', fontWeight: 600 }}>{formatVal(data[data.length-1]?.value)}</span></div>}

...
const AccountRecovery = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        setError('');
        try {
            const response = await axios.post('https://vercel-dataflow.vercel.app/api/accounts/account-recovery/', {
                email,
            });
            setMessage(response.data.message);
        } catch (error) {
            setError(error.response?.data?.error || 'Failed to send recovery email');
        }
    };
